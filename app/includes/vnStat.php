...
class vnStat
{
    ...
    public function getInterfaces()
    {
        ...
        foreach ($this->vnstatData['interfaces'] as $interface) {
            $vnstatInterfaces[] = $interface['name'];
        }
    ...
    public function getInterfaceData($timeperiod, $type, $interface)
    {
            ...
            if ($type === 'table') {
                foreach ($this->vnstatData['interfaces'][$arrayIndex]['traffic']['top' . $typeAppend] as $traffic) {
                    if (is_array($traffic)) {
                        ...
                        $trafficData = $this->getTableTrafficDataArr($traffic, $trafficData, $i);
        ...
        if ($timeperiod === 'hourly') {
            if ($type === 'table') {
                ...
                usort($trafficData, callback: "sortingFunction");
                ...
                foreach ($this->vnstatData['interfaces'][$arrayIndex]['traffic']['hour' . $typeAppend] as $traffic) {
                    if (is_array($traffic)) {
                        ...
                        $trafficData = $this->getGraphTrafficDataArr($traffic, $trafficData, $i);
                    }
                }
            }
        }
     ...
     * @return array
     */
    private function getTableTrafficDataArr(array $traffic, array $trafficData, int $i): array
    {
     ...
     * @return array
     */
    private function getGraphTrafficDataArr(array $traffic, array $trafficData, int $i): array
    {
